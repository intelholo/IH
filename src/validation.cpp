...
#if defined(NDEBUG)
# error "IH cannot be compiled without assertions."
#endif
...
bool fReindex = false;
bool fTxIndex = false;
bool fAddressIndex = false; //bitcore
...
const std::string strMessageMagic = "IH Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize)
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s IH additional fees, %d delta bytes\n",
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize)
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(500000000ull + ((pindex->nHeight - 5000) * 1ull)) * COIN){
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams, bool fBare = false)
{
    ...
    {
		bool fClean=true;  //bitcore
        CCoinsViewCache view(pcoinsTip);
		if (DisconnectBlock(block, state, pindexDelete, view, &fClean) != DISCONNECT_OK)    //bitcore
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for IH PoS we need to sync this after every block to ensure txdb is populated for
