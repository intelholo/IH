...
*Setup instructions for a Gitian build of IH Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IH
...
- [Building IH Core](#building-IH-core)
...
Clone the git repositories for IH and Gitian.
...
```bash
sudo cp /home/debian/IH/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the IH repository under 'Fetch and create inputs' to install sources which require
...
To build IH Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the IH repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/IH/.git/
...
For example:
```bash
URL=https://github.com/laanwj/IH.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit IH=${COMMIT} --url IH=${URL} ../IH/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit IH=${COMMIT} --url IH=${URL} ../IH/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit IH=${COMMIT} --url IH=${URL} ../IH/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-IH:
./bin/gbuild --commit IH=${COMMIT},cpp-eth-IH=${ETHCOMMIT} --url IH=${URL},cpp-eth-IH=${ETHURL} ../IH/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the IH git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../IH/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/IH
SIGPATH=/some/root/path/IH-detached-sigs

./bin/gbuild --url IH=${BTCPATH},signature=${SIGPATH} ../IH/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/IH-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/IH-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/IH-osx-build.assert
```
...
[IHproject/gitian.sigs](https://github.com/IHproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@IH.org) and he will commit them.
